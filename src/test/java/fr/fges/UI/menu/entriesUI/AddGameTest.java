...
public class AddGameTest {
    ...
    void shouldTellUserGameIsADuplicate() {
        ...
        when(service.addGame(any(BoardGame.class))).thenReturn(new Failure<>(new DuplicateGameException()));
    ...
    void shouldTellUserGameHasNotBeenSaved() {
        ...
        when(formatter.getNumberFromUser(anyString())).thenReturn(2);
        when(formatter.getGameCategory()).thenReturn("Family");
        when(service.addGame(any(BoardGame.class))).thenReturn(new Failure<>(new GameStorageException()));

        entry.handle(formatter);

        verify(formatter).displayMessage("An error occurred while saving game collection state, please try again.");
    }
}