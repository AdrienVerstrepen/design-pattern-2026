...
class ChampionshipFormatTest {
	...
	void setUp() {
		...
		format = new ChampionshipFormat("Championship (everyone plays everyone)", ui);
	}

	@Test
	void shouldReturnLabel() {
		assertEquals("Championship (everyone plays everyone)", format.label());
	}
	...
	@Test
	void shouldPlayTournamentWithChampionshipLogic() {
		Player p1 = new Player("Alice", 0, 0);
		Player p2 = new Player("Bob", 0, 0);
		...
		assertEquals(2, results.get(0).getPoints());
		...
		assertEquals(6, results.get(2).getPoints());
		assertEquals(2, results.get(2).getNumberOfWins());
		verify(ui, times(3)).getUserInput(anyString());
		verify(ui, atLeast(3)).displayMessage(contains("VS"));
	}
